identify_infection_normalized <- function(obj_list,
  ...
  viral_feat <- lapply(obj_list, function(obj) {
    rn   <- rownames(GetAssayData(obj, layer = "data"))
  ...
  obj_list <- mapply(function(obj, feats) {
    if (length(feats) == 0) {
      ...
      v       <- GetAssayData(obj, layer = "data")[feats, , drop = FALSE]
      total_v <- colSums(v)
  ...
  viral_feat_original <- lapply(obj_list, function(obj) {
    rn   <- rownames(GetAssayData(obj, assay = "original.counts", layer = "data"))
    hits <- unlist(lapply(viral_pattern, function(p) grep(p, rn, value = TRUE)))
    unique(hits)
  })
  ...
  obj_list <- mapply(function(obj, feats) {
    if (length(feats) == 0) {
      ...
      v       <- GetAssayData(obj, assay = "original.counts", layer = "data")[feats, , drop = FALSE]
      total_v <- colSums(v)
